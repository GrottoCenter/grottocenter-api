...
class GrottoMapClass extends Component {
  ...
  componentDidMount() {
    ...
    if (encodedParam !== undefined & encodedParam.length > 1) {
      ...
      let params = decoded.split("&").reduce(function(prev, curr, i, arr) {
        var p = curr.split("=");
  ...
  */
  ...
  handleEvent() {
    ...
    let encoded = btoa("lng=" + mapBounds.lng + "&lat=" + mapBounds.lat + "&zoom=" + zoom);
    window.location = window.location.origin + window.location.pathname + window.location.search + "#" + encoded;
  }
