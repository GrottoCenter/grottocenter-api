...
GC_LOCAL_PORT=1337
DOCKERFILE_NAME="DockerfileDemo"
...
docker build --memory-swap -1 -t ${GC_TAGNAME} -f ${DOCKERFILE_NAME} . || { echo '######## Build failed - Exiting now #########' ; exit 1; }

isServerContainerExisting="$(docker ps --all --quiet --filter=name="$GC_TAGNAME")"
if [ -n "$isServerContainerExisting" ]; then
  echo "### DELETING old server Container ###"
  docker stop $GC_TAGNAME && docker rm -f $GC_TAGNAME
fi
...
echo "###############################################################"
echo "###### Grottocenter demo ready at http://localhost:${GC_LOCAL_PORT} #######"
echo "###############################################################"
