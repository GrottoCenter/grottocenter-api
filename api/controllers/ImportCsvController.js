...
const getAuthor = async (data) => {
  let authorId;
  let author = doubleCheck(data, 'karstlink:hasDescriptionDocument/dct:creator', null);
  ...
  if(authorCaver.length === 0){
    ...
    const newCaver = await CaverService.createNonUserCaver(caverParams, handleError, esClient);
  ...
  const caversArray = await TCaver.find({
    nickname: creatorNickname,
        ...
        switch(caversArray.length){
          case 0:
            const [fullName, creatorName, creatorSurname] = getPersonData(creatorNickname);
            const paramsCaver = {
              name: creatorName,
              surname: creatorSurname,
              nickname: fullName,
            };
            caver = await CaverService.createNonUserCaver(paramsCaver, handleError, esClient);
          break;
          default:
...
const getConvertedCaveFromClient = (rawData, idAuthor) => {
  ...
  if(!depth){
    depth =  parseInt(doubleCheck(rawData, 'karstlink:extendBelowEntrance', 0), 10) + parseInt(doubleCheck(rawData, 'karstlink:extendAboveEntrance', 0), 10);
  }
  return {
    ...
    depth: depth,
    date_inscription: doubleCheck(rawData, 'dct:rights/dct:created', new Date()),
...
const getConvertedNameAndDescCaveFromClient = (rawData, authorId) => {
  return {
    ...
    descriptionAndNameLanguage: {
      id: doubleCheck(rawData, 'karstlink:hasDescriptionDocument/dc:language', undefined, iso2ToIso3),
    },
...
const getConvertedEntranceFromClient = (rawData, idAuthor, cave) => {
  return {
    ...
    nameDbImport: doubleCheck(rawData,'dct:rights/cc:attributionName', undefined),
...
const getConvertedNameDescLocEntranceFromClient = async (rawData, authorId) => {
  let result = {};
  if(doubleCheck(rawData, 'karstlink:hasDescriptionDocument/dct:title', undefined)){
    result = {
      description: {
        body: doubleCheck(rawData, 'karstlink:hasDescriptionDocument/dct:description', undefined),
        language: doubleCheck(rawData, 'karstlink:hasDescriptionDocument/dc:language', undefined, iso2ToIso3),
        title: doubleCheck(rawData, 'karstlink:hasDescriptionDocument/dct:title', undefined),
        author: authorId,
        dateInscription: doubleCheck(rawData, 'dct:rights/dct:created', new Date()),
        dateReviewed: doubleCheck(rawData, 'dct:rights/dct:modified', undefined),
      },
  ...
  if(doubleCheck(rawData, 'karstlink:hasAccessDocument/dct:description', undefined)){
    let authorLoc = authorId;
    const authorFromCsv = doubleCheck(rawData, 'karstlink:hasAccessDocument/dct:creator', undefined);
    if(authorFromCsv){
    ...
    result = {
      ...
      location: {
        ...
        title: doubleCheck(rawData, 'karstlink:hasAccessDocument/dct:description' , undefined),
        language: doubleCheck(rawData, 'karstlink:hasAccessDocument/dc:language', undefined, iso2ToIso3),
        author: authorLoc,
        dateInscription: doubleCheck(rawData, 'dct:rights/dct:created', new Date()),
        dateReviewed: doubleCheck(rawData, 'dct:rights/dct:modified', undefined),
      },
...
const getConvertedDocumentFromClient = async (rawData, authorId) => {
  const rawLicence = doubleCheck(rawData, 'dct:rights/karstlink:licenseType', undefined);
  ...
  if(licenceDb){
      ...
      editorName = editorName.slice(0,-2);
        ...
        switch(namesArray.length){
          case 0:
            ...
            const editorGrotto = await GrottoService.createGrotto(paramsGrotto, nameGrotto, handleError ,esClient);
    ...
    if(typeData){
      const typeCriteria = typeData.startsWith('http') ? {url:typeData} : {name: typeData};
    ...
    return {
      ...
      type: doubleCheck(rawData, 'karstlink:documentType', undefined, getDocumentType),
...
const getConvertedLangDescDocumentFromClient = (rawData, authorId) => {
  const desc = doubleCheck(rawData, 'karstlink:hasDescriptionDocument/dct:description', undefined);
  const langDesc = desc ? doubleCheck(rawData, 'karstlink:hasDescriptionDocument/dc:language', undefined, iso2ToIso3) : doubleCheck(rawData, 'dc:language', undefined, iso2ToIso3);
  return {
    author: authorId,
    title: doubleCheck(rawData, 'rdfs:label', undefined),
    description: doubleCheck(rawData, 'karstlink:hasDescriptionDocument/dct:description', undefined),
    dateInscription: doubleCheck(rawData, 'dct:rights/dct:created', new Date()),
    dateReviewed: doubleCheck(rawData, 'dct:rights/dct:modified', undefined),
 ...
 module.exports = {
    ...
    importAll: async (req, res) => {
      const requiredColumns = ['id', 'rdf:type', 'dct:rights/cc:attributionName','dct:rights/karstlink:licenseType','gn:countryCode'];
      const requiredLocationColumns = ['karstlink:hasAccessDocument/dct:description','karstlink:hasAccessDocument/dc:language','karstlink:hasAccessDocument/dct:creator'];
      const requiredDescriptionColumns = ['karstlink:hasDescriptionDocument/dct:title','karstlink:hasDescriptionDocument/dct:creator','karstlink:hasDescriptionDocument/dc:language'];
      ...
      switch(req.body.typeRow){
        case 'document':
          ...
          executeFunction = async (data) => {
            ...
            const dataDocument = await getConvertedDocumentFromClient(data, authorId);
            const dataLangDesc = getConvertedLangDescDocumentFromClient(data, authorId);
            const documentCreated = await DocumentService.createDocument(dataDocument, dataLangDesc, handleError);
            return {
        ...
        case 'entrance':
          ...
          executeFunction = async (data) => {
            ...
            const dataNameAndDesc = getConvertedNameAndDescCaveFromClient(data, authorId);
            const caveCreated = await CaveService.createCave(dataCave, dataNameAndDesc, handleError);
            
            //Entrance creation
            const dataEntrance = getConvertedEntranceFromClient(data, authorId, caveCreated);
            const dateInscription = dataEntrance.dateInscription;
            const dateReviewed = dataEntrance.dateReviewed;
            const dataNameDescLoc = await getConvertedNameDescLocEntranceFromClient(data, authorId);
            const entranceCreated = await EntranceService.createEntrance(dataEntrance, dataNameDescLoc, handleError, esClient);
      ...
      for(const [index, data] of req.body.data.entries()){
        ...
        if(doubleCheck(data, 'karstlink:hasDescriptionDocument/dct:title', false)){
        ...
        if(doubleCheck(data, 'karstlink:hasAccessDocument/dct:description', false)){
