...
module.exports = {
   ...
   * @returns {Promise<any>}
   */
  getQualityEntriesBetweenCoords: (southWestBound, northEastBound, qualityLimit) =>
    new Promise((resolve, reject) => {
      CommonService.query(PUBLIC_ENTRIES_IN_BOUNDS, [
        southWestBound.lat,
        northEastBound.lat,
  ...
  findByBoundsPartitioned: (southWestBound, northEastBound, qualityLimit) =>
    new Promise((resolve, reject) => {
      ...
      settingsPromiseList.push(
        SettingsService.getSetting('map.partition.group.minsize').then((value) => {
          minGroupSize = value;
        }),
      );
  ...
  getEntriesMap: (southWestBound, northEastBound, zoom, limitEntries) =>
    new Promise((resolve, reject) => {
      ...
      if (zoom > 11) {
        ...
        getEntriesPromiseList.push(
          GeoLocService.getQualityEntriesBetweenCoords(southWestBound, northEastBound, limitEntries)
            .then((entries) => {
              qualityEntriesMap = GeoLocService.formatQualityEntriesMap(entries);
            })
            .catch((err) => {
        ...
        getEntriesPromiseList.push(
          GeoLocService.findByBoundsPartitioned(southWestBound, northEastBound, limitEntries)
            .then((partResult) => {
        ...
        getEntriesPromiseList.push(
          GeoLocService.findByBoundsPartitioned(southWestBound, northEastBound, 0)
            .then((partResult) => {
              groupEntriesMap = partResult;
            })
            .catch((err) => {
      ...
      Promise.all(getEntriesPromiseList).then(() => {
        resolve(
          GeoLocService.formatEntriesMap(qualityEntriesMap, groupEntriesMap, grottoMap, caveMap),
        );
      });
    }),
};
