...
const PUBLIC_ENTRIES_COUNT_QUERY =
  "SELECT COUNT(id) AS count FROM t_entry WHERE Is_public='YES'";
...
module.exports = {
  ...
  completeRandomEntry: async (entryId) => {
    const entryData = await EntryService.findEntry(entryId);

    // add entry to result
    const completeEntry = { ...entryData[0] };

    // enrich result with entry info
    const entryInfo = await EntryService.getEntryInfos(entryId);
    if (entryInfo) {
