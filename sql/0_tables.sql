...
CREATE TABLE t_entrance (
	...
	CONSTRAINT t_entrance_type_check CHECK (
		(
			type = ANY (
				ARRAY ['entrance'::bpchar, 'loss'::bpchar, 'emerg'::bpchar, 'outcrop'::bpchar]
			)
		)
	),
...
CREATE TABLE t_point (
	...
	CONSTRAINT t_point_type_check CHECK (
		(
			type = ANY (ARRAY ['a'::bpchar, 'b'::bpchar, 'c'::bpchar])
		)
	),
...
CREATE TABLE t_document_duplicate (
	id serial NOT NULL,
	id_author int4 NOT NULL,
...
-- t_entrance_duplicate definition
-- Drop table
-- DROP TABLE t_entrance_duplicate;
CREATE TABLE t_entrance_duplicate (
	id serial NOT NULL,
	id_author int4 NOT NULL,
	content json NOT NULL,
	date_inscription timestamp NOT NULL DEFAULT now(),
...
-- j_document_massif definition
-- Drop table
-- DROP TABLE j_document_massif;
...
CREATE TABLE t_admin2 (
	...
	RUS varchar(200) NULL,
	ZHO varchar(200) NULL,
	CONSTRAINT t_admin2_pk PRIMARY KEY (id)
);
