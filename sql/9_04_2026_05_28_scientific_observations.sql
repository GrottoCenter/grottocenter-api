...
-- ============================================================
-- Scientific Observations Schema (Issue 1562 - BI POC)
-- ============================================================
...
-- Step 2: Drop the old t_point table
DROP TABLE IF EXISTS t_point;

-- Step 3: Recreate t_point with the new schema
...
-- Step 4: Re-add FK constraints pointing to the new t_point
...
CREATE TABLE IF NOT EXISTS t_observation (
  ...
  -- Denormalized fields (BI_ERD_CHANGES #1, #4)
...
-- Sensor (physical device / measurement channel)
CREATE TABLE IF NOT EXISTS t_sensor (
  ...
  CONSTRAINT t_sensor_t_quantity_kind_fk FOREIGN KEY (id_quantity_kind) REFERENCES t_quantity_kind(id)
);
...
CREATE TABLE IF NOT EXISTS t_sensor_configuration (
  ...
  CONSTRAINT t_sensor_configuration_t_sensor_fk FOREIGN KEY (id_sensor) REFERENCES t_sensor(id),
...
CREATE TABLE IF NOT EXISTS t_time_series (
  ...
  -- Denormalized fields (BI_ERD_CHANGES #2, #3)
...
-- Index for Superset time-range filtering (BI_ERD_CHANGES #5)
CREATE INDEX IF NOT EXISTS idx_time_series_date_range ON t_time_series (start_date, end_date);

-- Measurement (individual data points — partitioned by timestamp, BI_ERD_CHANGES #7)
...
-- Historical partitions for imported data
...
CREATE TABLE IF NOT EXISTS t_contamination (
  ...
  -- Denormalized field (BI_ERD_CHANGES #3)
...
-- ============================================================
-- Analytics view for Superset (Phase 1: direct view)
-- ============================================================
